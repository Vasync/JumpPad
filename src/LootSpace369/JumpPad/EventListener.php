  ...
  public function onMove(PlayerMoveEvent $ev) {
    ...
    foreach (Loader::getInstance()->getConfig()->get("JumpPad") as $pad) {
      ...
      if ($pos === $posCheck) {
        ...
        $this->lookAtLocation($player, (new Location((float)$to[0],(float)$to[1],(float)$to[2],\pocketmine\Server::getInstance()->getWorldManager()->getWorldByName($to[3]),(float)$to[4],(float)$to[5]))->add(0,0.5,0));
